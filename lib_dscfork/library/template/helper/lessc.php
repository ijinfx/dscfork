 ...
 * by `DSCForkTemplateHelperLessc_parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `DSCForkTemplateHelperLessc`. The CSS tree is fed into a
 * formatter, like `DSCForkTemplateHelperLessc_formatter` which then outputs CSS as a string.
 *
 * During the first compile, all values are *reduced*, which means that their
 * types are brought to the lowest form before being dump as strings. This
 * handles math equations, variable dereferences, and the like.
 *
 * The `parse` function of `DSCForkTemplateHelperLessc` is the entry point.
 *
 * In summary:
 *
 * The `DSCForkTemplateHelperLessc` class creates an intstance of the parser, feeds it LESS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `DSCForkTemplateHelperLessc_parser` class is only concerned with parsing its input.
 *
 * The `DSCForkTemplateHelperLessc_formatter` takes a CSS tree, and dumps it to a formatted string,
 * handling things like indentation.
 */
class DSCForkTemplateHelperLessc {
	 ...
	 *
	 * See DSCForkTemplateHelperLessc::compileProp()
	 *
	 */
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new DSCForkTemplateHelperLessc_parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new DSCForkTemplateHelperLessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "DSCForkTemplateHelperLessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "DSCForkTemplateHelperLessc_formatter_$this->formatterName";
		}
...
// syntax tree
class DSCForkTemplateHelperLessc_parser {
	 ...
	 *
	 * Consider the function DSCForkTemplateHelperLessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by DSCForkTemplateHelperLessc::match(), which behaves
	 ...
	 * avoid this, DSCForkTemplateHelperLessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function fixTags($tags) {
		...
		foreach ($tags as &$tag) {
			if ($tag{0} == $this->DSCForkTemplateHelperLessc->vPrefix)
				$tag[0] = $this->DSCForkTemplateHelperLessc->mPrefix;
		}
		return $tags;
	}
	...
	protected function tagBracket(&$value) {
		...
		if ($this->literal('[') && $this->to(']', $c, true) && $this->literal(']', false)) {
			...
			$value = str_replace($this->DSCForkTemplateHelperLessc->parentSelector, "$&$", $value);
			return true;
		}
	...
	protected function variable(&$name) {
		$s = $this->seek();
		if ($this->literal($this->DSCForkTemplateHelperLessc->vPrefix, false) &&
			($this->variable($sub) || $this->keyword($name)))
		{
			if (!empty($sub)) {
				$name = array('variable', $sub);
			} else {
				$name = $this->DSCForkTemplateHelperLessc->vPrefix.$name;
			}
			return true;
		}
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = DSCForkTemplateHelperLessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.DSCForkTemplateHelperLessc::preg_quote($what), $m, !$until)) return false;
...
class DSCForkTemplateHelperLessc_formatter_classic {
	public $indentChar = "  ";
...
class DSCForkTemplateHelperLessc_formatter_compressed extends DSCForkTemplateHelperLessc_formatter_classic {
...
class DSCForkTemplateHelperLessc_formatter_lessjs extends DSCForkTemplateHelperLessc_formatter_classic {
	public $disableSingle = true;
