...
class DSCForkTableNested extends DSCForkTable
{
	/**
	 * Extends to add some fields based on calculations
	 * @see sample/admin/tables/DSCForkTable#load($oid, $key)
	 */
	 ...
	 * @see sample/admin/tables/DSCForkTable#move($change, $where)
	 */
	...
	public function orderUp( $oid = null )
	{
		...
		$this->_db->setQuery( 'SELECT `' . $this->_tbl_key . '`' . ' FROM `' . $this->_tbl . '`' . ' WHERE `lft` BETWEEN ' . (int)$node->lft . ' AND ' . (int)$node->rgt );
	...
	public function orderDown( $oid )
	{
		...
		if( $sibling->parent_id != $node->parent_id || $sibling->$key == $node->$key )
		{
			$this->setError( JText::_( "LIB_DSCFORK_NODE_CANNOT_BE_ORDERED_ANY_LOWER" ) );
			$this->_unlock( );
			return false;
		}
	...
	function reorder( $parent = null, $where = '' )
	{
		...
		$query = 'SELECT ' . $this->_tbl_key . ', ordering' . ' FROM ' . $this->_tbl . ' WHERE ordering >= 0' . ($where ? ' AND ' . $where : '') . " AND parent_id = '{$parent}' " . ' ORDER BY ordering, lft ASC';
		$this->_db->setQuery( $query );
